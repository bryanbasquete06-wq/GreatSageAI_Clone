# -*- coding: utf-8 -*-
"""
Elívea — Ponto de Entrada Único
======================================
Restaura fluxo original que estava tão bom (usa elvea_app Elivea)
...
try:
    ...
    for _d in ["hf_cache","torch_cache","cache","elvea_tts","uploads","pip_cache","uv_cache","pycache","xtts","rag","logs","models"]:
        (_F_TEMP / _d).mkdir(parents=True, exist_ok=True)
    # garante que logs do app também vão para F
    os.environ["ELIVEA_LOG_DIR"] = str(_F_TEMP / "logs")
except Exception:
    pass
...
try:
    ...
    pass
# Terminal invisível é feito pelo Elívea.bat / Elivea_invisivel.vbs usando pythonw — sem console preto feio
...
def main():
    ...
    print("="*60)
    print("  Elívea — Elívea Class (restaurado)")
    print("  Iniciando via elvea_app original...")
    print("="*60)
    try:
        # tenta importar com fallback PySide6/PyQt6 já tratado em elvea_app
        from elvea_app import main as app_main
        app_main()
